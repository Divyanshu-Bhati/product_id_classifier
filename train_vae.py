...
class TrainVAE:
    def __init__(self):
        ...
        (self.max_length,
         ...
         self.y_val,
         self.char2idx,
         self.idx2char) = DataCreator(input_path=self.input_path, weights_path=self.weights_path, seed=self.random_seed). \
    ...
    def train_vae_model(self):
        ...
        # else: TODO separate inference, and make testing for the whole flow and not one subset -- how to evaluate VAE then? 
        #     print("Running VAE inference on test set (training data subset)...")
        #     weights_file = os.path.join(self.weights_path, "vae_weights.pth")
        #     vae_model.load_state_dict(torch.load(weights_file, map_location=self.device))
        #     print("Loaded model weights from", weights_file)

        # # Run inference on test set.
        # print("Running VAE inference on test set...")
        # inference = VAEInference(device=self.device)
        # inference.run_training_inference(model=vae_model,
        #                                  weights_path=self.weights_path,
        #                                  X_test_padded=self.X_test_padded)
    
if __name__ == "__main__":
    TrainVAE().train_model()